...
function Button({ children, variant = "primary", onClick, full, icon: Icon, type = "button" , disabled}) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} style={{ ...styles[variant], width: full ? "100%" : "auto", padding: variant === "text" ? 0 : "10px 16px", borderRadius: 8, fontSize: 13.5, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 7, fontFamily: FONT, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.6 : 1 }}>
      {Icon && <Icon size={14} />}
      {children}
    </button>
  );
